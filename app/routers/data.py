...
async def collect_weather_data(user_id: PydanticObjectId) -> WeatherDataResponse:
    ...
    if not user:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="User id does not exists."
        )
    else:
        previous_user_request = await WeatherData.find_one(
            WeatherData.user.id == user_id
        )
...
async def get_progress_percentage(user_id: PydanticObjectId) -> PercentageResponse:
    ...
    if not user:
        ...
        if not previous_user_request:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="User has not made any request.",
            )

        weather_data = await WeatherData.find(
            WeatherData.user.id == user_id, fetch_links=True
        ).first_or_none()
